...
@router.get("/")
def read_root():
    """Simple health endpoint for the sensor router."""
    return {"message": "Welcome to the sensor routes"}
...
def list_sensors(
    ...
    try:
        sensors = (
            db.query(SensorTable).order_by(SensorTable.date_time.desc()).offset(offset).limit(limit).all()
        )
...
def create_sensor(payload: SensorDTO, db: Session = Depends(get_db)):
    ...
    sensor = SensorTable(**sensor_data)
    try:
        db.add(sensor)
        db.commit()
        db.refresh(sensor)
